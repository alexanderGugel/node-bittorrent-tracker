...
const ANNOUNCEMENTS = ['started', 'completed', 'stopped', 'empty'];
...
function validateRequest(method, query) {
	...
	query['event'] = (query['event'] || 'empty');

	if (ANNOUNCMENTS.indexOf(query['event']) === -1) {
		throw new Failure(900, 'Invalid announcement (event)');
	}
}
